...
final class ComposeViewModel {
    ...
    /// The reply-bar sub-VM, created when the compose flow enters inline-reply mode.
    var replyBar: ReplyBarViewModel?

    var showCc = false
    var showBcc = false

    /// Convenience: delegates to ``ReplyBarViewModel/hasUserContent`` when in reply mode.
    /// Always `false` when there is no active reply bar (standalone compose uses view-local logic).
    var hasUserContent: Bool {
        replyBar?.hasUserContent ?? false
    }
    ...
    func autoSaveDraft(
        ...
        if let gid = gmailDraftID {
            ...
            if let threadID, mailStore.replyDrafts[threadID] != nil {
                let plain = await Self.stripHTML(bodyHTML)
