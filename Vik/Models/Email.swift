...
struct Contact: Identifiable, Hashable, Sendable {
    /// This ID is the lowercased email address.
    /// When combining contacts from different roles (sender, recipients, cc) in a single
    /// ForEach, use `enumerated()` with offset-based IDs to avoid collisions.
