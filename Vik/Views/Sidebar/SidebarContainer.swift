...
struct SidebarContainer: View {
    ...
    /// Isolates reads from `MailboxViewModel` (`userLabels`, `folderUnreadCounts`)
    /// so that changes to other VM properties (e.g. `emails`) don't invalidate
    /// SidebarContainer's parent.
    private struct SidebarDataExtractor: View {
        let coordinator: AppCoordinator
        @Binding var isSidebarCollapsed: Bool
        var appFocus: FocusState<AppFocus?>.Binding
        let sidebarWidth: CGFloat
