...
struct CreateCalendarEventIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        let event = CalendarAPIEventInput(
            ...
            description: nil,
            location: location,
            start: CalendarAPIDateTime(date: nil, dateTime: startDate.rfc3339String, timeZone: TimeZone.current.identifier),
            end: CalendarAPIDateTime(date: nil, dateTime: resolvedEnd.rfc3339String, timeZone: TimeZone.current.identifier),
            attendees: nil,
            reminders: nil,
            conferenceData: nil,
            colorId: nil,
