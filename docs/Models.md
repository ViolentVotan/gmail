...
| `SnoozePreset` *(in `Views/Common/SnoozePickerView.swift`)* | `Identifiable` struct with `id`, `title`, `icon`, `date` fields. `static var tomorrowMorning: Date` provides the default snooze date (tomorrow 8:00 AM), reused by `defaults()` and `HoverActionButtonsView`. `static func defaults()` returns the standard preset list (later today, tomorrow, next week). Defined alongside its view for locality but functions as a shared data model. |
