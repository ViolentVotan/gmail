...
| File | Role |
|------|------|
| `Email.swift` | UI-facing email model (computed from GmailMessage). Custom `Equatable` comparing only identity (`id`, `gmailMessageID`) + UI-relevant mutable fields (`isRead`, `isStarred`, `labels`, `gmailLabelIDs`, `preview`, `threadMessageCount`, `tags`, `isDraft`, `isGmailDraft`, `gmailDraftID`) — excludes `body`/`subject`/`sender`/`attachments` for performance. `Hashable` conformance uses `id` + `gmailMessageID` (matching Equatable identity fields). `Contact.id` is computed from `email.lowercased()` for deterministic equality. |
