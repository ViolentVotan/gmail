...
| `BIMIService.swift` | BIMI logo resolution via DNS-over-HTTPS. `resolveBIMI` is `@concurrent private`. Cache capped at 500 entries with 25% partial eviction. |
...
| `SnoozeMonitor.swift` | Background timer that un-snoozes emails when their snooze-until date arrives. Errors are logged via `print` and tracked via per-item failure counts; a toast is shown after `failureNotifyThreshold` (5) consecutive failures. |
