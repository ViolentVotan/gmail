...
- `GmailProfileService` — Gmail profile info, user identity (OAuth userinfo), send-as aliases, signature management. `getUserInfo` has its own retry loop with `RetryPolicy` (retries network errors, 403 rate-limit, and retriable status codes with exponential backoff + Retry-After).
...
- `GmailDraftService` — Draft fetch (single + batch via `batchFetch` with format-specific `fields` via `DraftFields` enum); `getDrafts` returns `(drafts: [GmailDraft], failedIDs: [String])` tuple. `createDraft`/`updateDraft` accept `bcc` parameter. Used for quick reply draft loading.
...
| `SpotlightIndexer.swift` | CoreSpotlight email indexing for Spotlight search. `indexEmail` is `async` (sequential index + prune, no fire-and-forget races). Deletes all items before indexing when `indexedCount >= maxIndexed` (1000), resetting `indexedCount` to 0. |
