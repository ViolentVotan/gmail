...
- `GmailAPIClient` — HTTP layer (auth headers, base URL, per-account token refresh coalescing via `refreshAndRetry`/`performRefresh`, retry with exponential backoff + Retry-After on 429/500/503 capped at 32s, 401 auto-retry on standard and batch requests, generic `batchFetch<T>()` for batch GET operations with `@concurrent fetchSingleBatch`, `requestURL()` for non-Gmail Google APIs, `Logger`-based diagnostics). Per-part 429 retry in batch responses via `retryRateLimitedParts` (exponential backoff, max 3 attempts). Batch requests track the active access token mutably so a 401 refresh propagates to subsequent 429 retries. `GmailAPIError` includes `dailyLimitExceeded` and `domainPolicy` cases. Batch response parsing strips the `response-` prefix from Content-IDs. `GmailPathBuilder` utility enum provides `queryAllowed` charset (internal), `labelQueryParam`, `sendAsPath`.
...
| `NotificationService.swift` | `UNUserNotificationCenter` push notifications with reply/archive/mark-read actions. ARCHIVE and MARK_READ action handlers fall back to `OfflineActionQueue` on API failure. Uses `Logger` (not print). |
