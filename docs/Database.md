...
| `CacheMigration.swift` | One-time JSON cache → GRDB migration (labels, messages, AI tags). Runs on first launch per account. `migrateTags` is `async throws` — systemic DB write failures propagate to `migrateIfNeeded`, which logs and returns early without setting the completion flag (retried on next launch). Completion flag set after cleanup succeeds (not before). Flag cleared on account removal. Tag migration relies on FK constraints to reject orphaned tags (no per-record existence check). |
