...
| `HTMLTemplate.swift` | HTML email rendering templates with CSS sanitization (`sanitizeCSSValue()` applied to all color interpolations) and HTML sanitization (case-insensitive stripping of `<script>`, `<style>`, `<iframe>`, `<object>`, `<embed>`, `<form>`, `<base>`, `<meta http-equiv>`, event handlers, `javascript:` URLs, `data:text/html` URIs via multi-line regex with `[\\s\\S]*?`) and nonce-based Content-Security-Policy (`style-src 'unsafe-inline'`). ARIA accessibility: editor div has `role="textbox"`, `aria-multiline`, `aria-label`; `a11y-status` div with `aria-live="polite"` announces formatting changes to screen readers. |
...
| `UserActivityManager.swift` | `NSUserActivity` builder for Siri onscreen content awareness. Creates activities with `CSSearchableItemAttributeSet` (subject, sender, recipients) for the `com.vikingz.serif.viewEmail` activity type. Used by `EmailDetailView` and `ContentView`. |
