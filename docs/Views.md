...
- `ThreadMessageCardView` — Individual message card with quote stripping toggle, click-to-show sender info popover (`.onTapGesture`, `.pointerStyle(.link)`). Hover background animated (`.snappy(duration: 0.2)`). Collapsed cards sit at 0.99 scale, expanding to 1.0 for subtle depth cue.
