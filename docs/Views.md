...
- `ThreadMessageCardView` — Individual message card with quote stripping toggle, click-to-show sender info popover (`.onTapGesture`, `.pointerStyle(.link)`). Uses Liquid Glass (`.glassEffect(.regular.interactive())`) for expanded and hovered cards, `.identity` when idle. Thread cards are wrapped in `GlassEffectContainer(spacing: 1)` for blended stacking. HTML email body uses `isContentLoaded` binding from `HTMLEmailView` for smooth opacity fade-in (`VikAnimation.contentSwitch`).
