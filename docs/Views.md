...
### `Sidebar/`
Left column — fixed-width, collapsible. **Expanded** (240pt): `List(.sidebar)` with folder navigation (Inbox is a flat button — category filtering lives in `CategoryTabBar`), account switcher, labels (8pt circle color indicators, `Section("Labels")` header), context menus. **Collapsed** (52pt icon-only): folder SF Symbol icons with Liquid Glass hover states (`GlassEffectContainer`, `.glassEffect(.regular.interactive())` on hover/selection, `.identity` when idle), compact account avatar, icon-only sync bubble, and labels popover button (tag icon, `.trailing` popover). Toggle via `⌘\` or toolbar button; `onToggleSidebar` callback wired from `ContentView`. Collapse/expand uses symmetric `.opacity` transitions with `VikAnimation.springDefault`. `AccountSwitcherView` uses closure-based callbacks (`onSetAsDefault`, `onSetAccentColor`) wired from `SidebarView` — no direct `AccountStore` access. `isExpanded: false` on `AccountSwitcherView` in collapsed mode triggers `onExpandSidebar` on avatar tap. A Mail/Calendar segmented control at the top of the sidebar switches between `AppViewMode.mail` and `.calendar`, updating `AppCoordinator.viewMode`.
...
- `ReplyBarView` — Thin rendering shell for inline quick reply. Delegates all draft/send/content state to `ReplyBarManager` (see ViewModels). View-local state: `isExpanded`, `showDiscardAlert`, `showTranslation`, `isEditorFocused`, `sendHapticTrigger`, `editorState`. Features: `AutocompleteTextField` for To/Cc/Bcc fields (glass identity rule on Cc/Bcc toggles), collapsible subject row with override support, `ScheduleSendButton` for deferred delivery, draft persistence with auto-save, discard confirmation, compose-side translation, `SuggestionChipRow` for smart reply and AI quick reply chips, draft loading shimmer overlay, AI reply generation placeholder, error banner above action bar (dismissible, multi-line), offline indicator (`NetworkMonitor.shared.isConnected`). `.task(id: email.id)` triggers `manager.resetEmail(...)`. Send success driven reactively via `.onChange(of: manager.composeVM.isSent)`. `ClickOutsideDetector` uses `superview` bounds for hit testing. Corner radius: `CornerRadius.md` per design concept.
...
- `HTMLEmailView` — WKWebView-based HTML email renderer (`NSViewRepresentable`). Uses `PassthroughWebView` (scroll-through subclass), `WeakScriptMessageHandler` (prevents WKWebView retain cycles), bidirectional WCAG contrast enforcement, and Live Text disabled via private SPI. `Coordinator` handles navigation delegation and JS bridge. Email content is injected via `callAsyncJavaScript` with parameter binding after the shell page loads — never string-interpolated into the page source (JS parameter binding security pattern). Exposes `@Binding var isContentLoaded: Bool` — set to `false` on content load start, `true` on `didFinish` navigation — enabling parent views to fade in content smoothly.
...
- `CalendarContainerView` — Root container. Switches between week/day/agenda sub-views based on `CalendarViewModel.viewMode` using a conditional content swap with `VikAnimation.contentSwitch`. Hosts `CalendarHeaderView` pinned at the top. Threads event action callbacks (`onEdit`, `onDelete`, `onRSVP`, `onEmailAttendees`) to all sub-views for context menus.
...
| `KeyboardShortcutsView` | Responder-aware keyboard event monitor (`KeyboardEventMonitor` NSViewRepresentable with `Coordinator`) for global shortcut handling. Handles calendar-mode shortcuts: `D` (day view), `W` (week view), `A` (agenda view), `E` (edit selected event), `Y` (accept invite), `M` (maybe), and numeric jump keys. |
