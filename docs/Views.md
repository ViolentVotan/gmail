...
### `Sidebar/`
Left column — fixed-width, collapsible. **Expanded** (220pt): `List(.sidebar)` with folder navigation, account switcher, labels, context menus. **Collapsed** (52pt icon-only): folder SF Symbol icons with selection highlight, compact account avatar, icon-only sync bubble. Toggle via `⌘\` or toolbar button; `onToggleSidebar` callback wired from `ContentView`. Labels section hidden when collapsed. `AccountSwitcherView` uses closure-based callbacks (`onSetAsDefault`, `onSetAccentColor`) wired from `SidebarView` — no direct `AccountStore` access. `isExpanded: false` on `AccountSwitcherView` in collapsed mode triggers `onExpandSidebar` on avatar tap.
...
### `EmailList/`
Middle column — fixed 380pt width (not user-resizable). Email rows with native `.swipeActions()` (archive/delete), search, `.refreshable` pull-to-refresh, multi-select with bulk actions. Uses `List(selection:)` for row rendering. Date-based sort orders show section headers (Today, Yesterday, This Week, Last Week, month/year). Email rows merge Gmail labels and AI classification tags into a single capped badge row (max 2 visible + overflow count). `EmailRowView` caches `nudgeText` as a stored `let` property (computed once in `init`, not per-render) for "Received N days ago" hints. Accessibility rotor filter properties are inlined into rotor closures (no separate stored properties). `EmailListView` has a midnight refresh `.task` that sleeps until the next midnight and triggers a re-render so "Today"/"Yesterday" section headers stay correct. Priority filter toggle (flag icon, glass background) lives in `EmailListView`'s search header row next to the sort menu (shown only for inbox).
- `CategoryTabBar` — Horizontal tab bar for inbox category filtering (All, Primary, Social, Promotions, Updates, Forums). 8pt tab padding; `ScrollView` fallback for narrow windows.
